    ...
    const fetchProducts = async () => {
      try {
        const response = await apiConnector(
          "GET",
          `${productEndpoints.GET_ALL_PRODUCT_API}?categoryId=689df3685af9247fe9656796`
        );
        setProducts(response?.data?.products || []);
      } catch (error) {
        console.error("Error fetching East Indian products:", error);
      }
    };
  ...
  return (
    <motion.div className="p-6">
      <motion.h1
        ...
        animate="visible"
      >
        Savor the Flavors of East India
      </motion.h1>
